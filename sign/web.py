...
def help(signtype):
    return f"""
    {signtype.rjust(20, ' ')}sign a web certificate.
                        req: certificate request file for sign
                        Ex: curl {conf.config['ListenHost']}/sign/{signtype} -F 'req=@<req file path>'"""
    
def checktype(cn):
    return !json.loads(os.popen(f'openssl x509 -in {certpath} -text | grep -A 1 "Basic Constraints:" | grep "CA" | sed \'s/\s//g\' | awk -F \':\' \'{print $2}\'').read().strip().lower())

def sign(request):
    reqname = f'/tmp/{str(uuid.uuid4())}.req'
    request.files['req'].save(reqname)
    ...
    for name in subaltname:
        ...
        try:
            ...
            elif conf.getipindex(request.remote_addr) < 0:
                return "Invalid IP address.", 403
            elif conf.getipindex(str(dns.resolver.resolve(str(dns.resolver.resolve(re.sub(r'DNS:[^.]*\.', '', name),'NS')[0]), 'A')[0])) != conf.getipindex(request.remote_addr):
                os.remove(reqname)
